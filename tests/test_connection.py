...
from aiortsp.rtsp.auth.digest import parse_digest_header
...
async def test_client_connection():
    ...
    server = await asyncio.start_server(
        functools.partial(handle_client, queue), "127.0.0.1", 5554
    )
    try:
        ...
        for msg in parser.parse(data):
            ...
            if "authorization" in msg.headers:
                ...
                params = parse_digest_header(
                    msg.headers["authorization"].split(" ", 1)[-1]
                )
                print("PARAMS", params)
                if (
                    params.get("nonce")
                    == "0024e47aY398109708de9ccd8056c58a068a59540a99d3"
                    and params.get("realm") == "AXIS_ACCC8E000AA9"
                    and params.get("opaque") == "123soleil"
                    and params.get("uri") == "rtsp://cam/axis-media/media.amp"
                    and params.get("username") == "root"
                    and params.get("response") == "7daaf0f4e40fdff42cff28260f37914d"
                ):
                    authorized = True
        ...
        async with RTSPConnection("127.0.0.1", 5554) as conn:
            with pytest.raises(RTSPResponseError):
                await conn.send_request(
                    "DESCRIBE", "rtsp://cam/axis-media/media.amp", timeout=2
                )
    finally:
        server.close()
...
async def test_client_auth_invalid_credentials():
    ...
    try:
        async with RTSPConnection(
            "127.0.0.1", 5554, username="toto", password="hello"
        ) as conn:
            with pytest.raises(RTSPResponseError):
                await conn.send_request(
                    "DESCRIBE", "rtsp://cam/axis-media/media.amp", timeout=2
...
async def test_client_auth():
    ...
    try:
        async with RTSPConnection(
            "127.0.0.1", 5554, username="root", password="admin123"
        ) as conn:
