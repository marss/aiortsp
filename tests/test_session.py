...
async def handle_client_auth(client_reader, client_writer):
    ...
    while True:
        ...
        for msg in parser.parse(data):
            ...
            if playing:
                # Send 2 RTP packets
                rtp = bytearray.fromhex(
                    "2400002080605eaac639ab5e13cd9b86674d0029e29019077f1180b7010101a41e244540"
                    "2400001080605eabc639ab5e13cd9b8668ee3c80"
                )
                client_writer.write(rtp)

                # Send an SR
                sr = bytearray.fromhex(
                    "80c8000677ae8d65e051bc2bea33b0001fa8034c0000000000000000"
                )
        ...
        async with RTSPConnection("127.0.0.1", 5554, timeout=2) as conn:
            async with TCPTransport(conn) as transport:
                async with RTSPMediaSession(
                    conn, "rtsp://cam/media.sdp", transport
                ) as sess:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            1580000040,
            1580000000,
            "clock=20200126T005400Z-20200126T005320Z",
