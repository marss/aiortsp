...
async def main():
    ...
    parser.add_argument(
        "-a", "--address", default="127.0.0.1", help="destination UDP address"
    )
    parser.add_argument("-A", "--auth", type=str, help="Auth to force ")
    parser.add_argument(
        "-p", "--props", default=None, help="Stream props (guessed if not provided)"
    )
    ...
    async with RTSPConnection(
        p_url.hostname, p_url.port or 554, p_url.username, p_url.password, logger=logger
    ) as conn:
        ...
        async with transport_class(
            conn, logger=logger, timeout=args.timeout
        ) as transport:
