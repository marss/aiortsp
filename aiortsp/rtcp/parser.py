...
class RTCPPacket:
    ...
    def pack(self, count, value, pad=False) -> bytes:
        ...
        if not pad or len(value) % 4 == 0:
            ...
            padding = b"\x00" * (4 - len(value) % 4 - 1) + pack(
                "!B", 4 - len(value) % 4
            )
...
class SRReport:
    ...
    def __repr__(self):
        return (
            f"<SRReport ssrc={self.ssrc} "
            f"seq={self.hseq} flost={self.flost}/256 "
            f"cumul={self.clost} jitter={self.jitter}>"
        )
...
class SR(SRRR):
    ...
    def __init__(
        self, ssrc, ntp, ts=0, pkt_count=0, byte_count=0, extn=None, reports=None
    ):
    ...
    def __repr__(self):
        return (
            f"<SR ssrc={self.ssrc} ts={self.ts} pkt={self.pkt_count} "
            f"ntp={self.ntp} reports={self.reports}>"
        )

    def __bytes__(self):
        ntp1, ntp2 = ts_to_ntp(self.ntp)
        value = pack(
            "!IIIIII", self.ssrc, ntp1, ntp2, self.ts, self.pkt_count, self.byte_count
        )
...
class APP(RTCPPacket):
    ...
    def __repr__(self):
        return (
            f"<APP stype={self.subtype} ssrc={self.ssrc} "
            f"name={self.name} data_len={len(self.data)}>"
        )
    ...
    def unpack(cls, data: bytes) -> "RTCP":
        ...
        while data:
            ...
            if payload_length > len(data):
                raise ValueError(
                    f"RTCP Packet truncated ({payload_length} > {len(data)})"
                )
